#!/usr/bin/env python3
"""Build the GraphHopper Internals Handbook HTML site from the book's markdown.
...
REPO_URL = "https://github.com/rumitvn/graphhopper-book"
REPO_BLOB = REPO_URL + "/blob/main"
UPSTREAM = "graphhopper/graphhopper"
PINNED = "11.0"

# ---- ordered pages -----------------------------------------------------------
CHAPTERS = [
    ("00", "00-environment-and-java-refresher.md", "Environment & Java Refresher", "Foundation",
    ...
    ("01", "01-mental-model-and-the-repo-map.md", "Mental Model & the Repo Map", "Foundation",
    ...
    ("02", "02-the-graph-map-to-network.md", "The Graph — turning a map into a routable network", "Core",
    ...
    ("03", "03-shortest-path-algorithms.md", "The Shortest-Path Algorithm Family", "★ Flagship",
    ...
    ("04", "04-profiles-weighting-custom-models.md", "Profiles, Weighting & Custom Models", "Core",
    ...
    ("05", "05-public-transit-gtfs-raptor.md", "Public Transit — GTFS & RAPTOR", "Application",
    ...
    ("06", "06-map-matching.md", "Map-Matching — snapping GPS to roads", "Application",
    ...
    ("07", "07-navigation-isochrones-web.md", "Navigation, Isochrones & the Web/Tiles Surface", "Application",
    ...
    ("08", "08-capstone-and-staying-current.md", "Capstone & Staying Current", "Mastery",
...
LANG_LABEL = {"c": "C", "cpp": "C++", "bash": "bash", "sh": "bash", "json": "json",
              ...
              "cmake": "CMake", "make": "Makefile", "diff": "diff",
...
def convert(md_path, single=False, prefix=None):
    ...
    for hm in re.finditer(r'<h([23])(?: class="[^"]*")? id="([^"]+)">(.*?)</h\1>', body, flags=re.S):
        lvl, hid, txt = hm.group(1), hm.group(2), html.unescape(re.sub(r"<[^>]+>", "", hm.group(3)).strip())
        toc.append((lvl, hid, txt))

    title = re.search(r"<h1[^>]*>(.*?)</h1>", body, flags=re.S)
    title = html.unescape(re.sub(r"<[^>]+>", "", title.group(1)).strip()) if title else "Handbook"
...
def rail_html(active):
    out = ['<aside class="rail"><nav>',
           ...
           '<span class="bm-pub">GraphHopper Internals Handbook</span></a>',
           '<h4>Chapters</h4>']
...
<title>{html.escape(title)} · GraphHopper Internals Handbook · RumitX</title>
<meta name="description" content="From-the-source handbook on graphhopper/graphhopper: how an open-source routing engine turns a map into a graph and finds the fastest route — Dijkstra, A*, Contraction Hierarchies, GTFS/RAPTOR transit, and map-matching.">
...
<header class="header">
  ...
  <a class="brand" href="index.html"><span class="mark"><span>X</span></span> GraphHopper Internals Handbook</a>
...
<footer class="site-foot">
  ...
  GraphHopper Internals Handbook · grounded in <a href="{REPO_URL}" target="_blank" rel="noopener">{UPSTREAM}</a>
  @ <code>{PINNED}</code> · maps &amp; route-finding, from the source<br>
  A <a href="https://rumitx.com" target="_blank" rel="noopener">RumitX</a> publication · maps, routing &amp; human-centric mobility
</footer>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>GraphHopper Internals Handbook · maps &amp; route-finding from the source · RumitX</title>
<meta name="description" content="A from-the-source handbook on graphhopper/graphhopper: the graph model, Dijkstra / A* / Contraction Hierarchies / Landmarks, custom-model weighting, GTFS &amp; RAPTOR transit, map-matching, and the routing web API.">
<link rel="icon" type="image/svg+xml" href="assets/brand/logo_x.svg">
<meta name="author" content="RumitX">
<meta name="theme-color" content="#0B1E2D">
<link rel="stylesheet" href="assets/styles.css">
...
<section class="hero">
  <span class="kicker">● GraphHopper · open-source routing engine · maps &amp; route-finding</span>
  <h1>Find the route,<br>not just draw the map.</h1>
  <p class="sub">A from-the-source handbook that turns <code>{UPSTREAM}</code> — the open-source engine that
  powers routing, transit and navigation for real mobility apps — into something you genuinely understand.
  Follow one <code>/route</code> request from the HTTP API, into the <code>BaseGraph</code>, through
  <code>Dijkstra</code> → <code>A*</code> → <code>Contraction Hierarchies</code>, across
  <code>GTFS</code>/<code>RAPTOR</code> transit and GPS <code>map-matching</code> — every step grounded in
  real <code>file:line</code> citations.</p>
  <div class="chips">
    <span class="chip">9 chapters · 1 flagship</span>
    <span class="chip">pinned <b>{PINNED}</b></span>
    <span class="chip">OSM → graph → <b>CH → route</b></span>
    <span class="chip">reconnects <b>BusMap / VinBus &amp; ride-hailing</b></span>
  </div>
  <div class="cta">
    <a class="btn primary" href="00-environment-and-java-refresher.html">Start with Chapter 0 →</a>
    <a class="btn ghost" href="03-shortest-path-algorithms.html">Jump to the flagship ★</a>
  </div>
</section>
<main class="landing">
  <div class="section-label">The curriculum — depth weighted toward the shortest-path algorithms</div>
  <div class="bento">
    {cards_html}
  </div>
  ...
  <div class="bento">
    <div class="card wide"><span class="cnum">METHOD</span><h3>Read with the source open</h3>
      ...
      Keep <code>graphhopper</code> open beside each page. Each chapter ends with checkpoint questions and a
      “Connect to your past” sidebar bridging to transit apps (BusMap, VinBus) and ride-hailing routing.</p></div>
    <div class="card"><span class="cnum">SHAPE</span><h3>Why → model → source → lab</h3>
      <p>Concept, then a mental model + ASCII diagram, then a guided source read, then a hands-on lab you run
      against your own local GraphHopper server with a stated expected result.</p></div>
  </div>
</main>
<footer class="site-foot">
  <a class="foot-brand" href="https://rumitx.com" target="_blank" rel="noopener"><span class="foot-x">X</span> RumitX</a>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>GraphHopper Internals Handbook — single file · RumitX</title>
<meta name="description" content="Complete from-the-source handbook on graphhopper/graphhopper — open-source routing internals from map data to the fastest route, transit and map-matching, one file.">
<meta name="author" content="RumitX">
<meta name="theme-color" content="#0B1E2D">
<link rel="icon" type="image/svg+xml" href="assets/brand/logo_x.svg">
<style>{styles}
...
<header class="header">
  <button class="icon-btn" id="menu-btn" aria-label="Open navigation">{MENU}</button>
  <a class="brand" href="#top"><span class="mark"><span>X</span></span> GraphHopper Internals Handbook</a>
  <span class="spacer"></span>
...
<div class="shell" style="grid-template-columns: var(--w-rail) minmax(0,1fr);">
  ...
  <main class="content">
    <section id="top" class="hero" style="text-align:left;max-width:var(--maxw-content);margin:0;padding-top:1rem;">
      <span class="kicker">● GraphHopper · open-source routing engine · maps &amp; route-finding</span>
      <h1 style="font-size:var(--text-h1);">GraphHopper Internals Handbook</h1>
      <p class="sub" style="margin-left:0;">The complete book in one file — the <code>BaseGraph</code>, the
      shortest-path family (<code>Dijkstra</code>, <code>A*</code>, <code>Contraction Hierarchies</code>,
      <code>Landmarks</code>), custom-model weighting, <code>GTFS</code>/<code>RAPTOR</code> transit,
      GPS <code>map-matching</code>, and the routing web API.
      Grounded in <code>{UPSTREAM}</code> @ <code>{PINNED}</code>. Use the rail to jump; press the printer icon to save as PDF.</p>
    </section>
    {sections_html}
  </main>
</div>
<footer class="site-foot">
<a class="foot-brand" href="https://rumitx.com" target="_blank" rel="noopener"><span class="foot-x">X</span> RumitX</a>
GraphHopper Internals Handbook · single-file edition · grounded in
<a href="{REPO_URL}" target="_blank" rel="noopener">{UPSTREAM}</a> @ <code>{PINNED}</code><br>
